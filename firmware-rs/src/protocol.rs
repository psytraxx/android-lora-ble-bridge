...
/// Format bytes as hex string for debugging (FR-007)
/// Example: [0x01, 0x02, 0xFF] -> "01 02 FF"
pub fn hex_dump(data: &[u8]) -> heapless::String<256> {
    ...
    for (i, byte) in data.iter().enumerate() {
        ...
        let hex = if *byte < 16 {
            heapless::String::<2>::try_from(alloc::format!("0{:X}", byte).as_str())
        } else {
            heapless::String::<2>::try_from(alloc::format!("{:X}", byte).as_str())
        };
        if let Ok(hex_str) = hex {
            let _ = result.push_str(&hex_str);
        }
        // Limit to prevent overflow (shows first ~80 bytes)
        if result.len() > 240 {
